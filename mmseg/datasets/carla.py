...
class CarlaDataset(CustomDataset):
    ...
    def load_annotations(self, img_dir, _0, ann_dir, _1, _2):
        ...
        img_infos = []
        for img in glob.glob(os.path.join(img_dir, '**/rgb_v/*.png'), recursive=True):
            relpath = os.path.relpath(img, img_dir)
            img_info = dict(filename=relpath)
            if ann_dir is not None:
                seg_map = relpath.replace('rgb_v', 'mask_id_v')
